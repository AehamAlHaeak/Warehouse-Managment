    ...
    public function handle(): void
    {
        ...
        try {
            ...
            foreach ($this->validated_products as $product) {
                ...
                $expiryDate = Carbon::parse($imported_product->expiration);
                $delay = $expiryDate->diffInSeconds(now());
                $delay_by_days=$delay/60/60/24;
                Log::info("the product :  $imported_product will expire after  $delay_by_days ");
                expiration::dispatch($imported_product->id)->delay(now()->addSeconds($delay));
                if (!$parent_continer) {
